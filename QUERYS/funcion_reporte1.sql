CREATE OR REPLACE FUNCTION "public"."reporte_estadis" ()  RETURNS TABLE(super_entidad character varying, id_entidad integer, codigo character varying, entidad character varying)
  VOLATILE
AS $dbvis$
BEGIN
    ...
    create temp table dos as
      Select uno.super_entidad,uno.id_entidad,uno.codigo,uno.entidad,
      case  
           when id_sistema=7 
	...
	select dos.super_entidad ,dos.id_entidad,dos.codigo,dos.entidad,max(spo) spo, max(soa) soa, max(sp) sp,max(sap) sap, max(sabs) sabs, max(st) st,max(scp) scp, max(sci) sci, max(rce) rce,max(srip)srip
	from dos
	group by dos.super_entidad ,dos.id_entidad,dos.codigo,dos.entidad;
